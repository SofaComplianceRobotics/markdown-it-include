...
test('default.txt', () => {
  ...
  runFixture(path.join(__dirname, 'fixtures/default.txt'), md);
});

test('including same field twice', async () => {
  const md = markdown()
    .use(markdown_it_include, fixturesPath);
...
test('accepts C-like includes with custom RE', () => {
  const md = markdown()
    .use(markdown_it_include, options);
  runFixture(path.join(__dirname, 'fixtures/incM.txt'), md);
});

test('barfs on illegal include statement without space(s)', () => {
  const md = markdown()
    .use(markdown_it_include, options);
...
test('barfs on missing braces when option says they\'re mandatory (default)', () => {
  const md = markdown()
...
test('dumps error in the generated output when throwError option is FALSE', () => {
  const md = markdown()
...
test('accepts a dynamic option.getRootDir() function to load includes from active directory', () => {
  const options2 = {
